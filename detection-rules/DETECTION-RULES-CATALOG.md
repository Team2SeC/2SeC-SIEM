...
└────────────────────┬────────────────────────────────────────┘
                     ...
                     ▼
┌─────────────────────────────────────────────────────────────┐
│        Level 1: Classification (Logstash Filter)             │
│                                                               │
│  • 단일 이벤트 기준 공격 유형 분류                                │
...
### Level 1: Classification (Logstash)
...
**구현 위치**: `logstash/config/logstash.config` (Section 8)
...
**구현 위치**: `detection-rules/opensearch-behavior-detection.json`
...
| 필드명 | 생성 위치 | 타입 | 설명 | 예시 값 |
|--------|----------|------|------|---------|
| `attack.rule_id` | Logstash | keyword | 탐지 룰 ID | `WEB-SQLI-001` |
| `attack.type` | Logstash | keyword | 공격 유형 | `SQL Injection` |
| `attack.category` | Logstash | keyword | 공격 카테고리 | `Injection` |
| `attack.severity` | Logstash | keyword | 심각도 | `low`, `medium`, `high`, `critical` |
| `attack.confidence` | Logstash / OpenSearch | keyword | 신뢰도 | `low`, `medium`, `high` |
| `detection.stage` | Logstash / OpenSearch | keyword | 탐지 단계 | `classification`, `detection` |
| `detection.source` | Logstash / OpenSearch | keyword | 탐지 주체 | `logstash`, `query` |
